...
export default function Letters() {
    ...
    return (
        <main className="flex justify-center items-center h-screen bg-[#2b2d42]">
            <section className="w-[80%] h-[80%] bg-[#edf2f4] rounded border-8 border-[#ef233c] flex flex-col overflow-scroll">
                <h1 className="flex justify-center text-3xl font-extrabold text-[#2b2d42] mt-2">
                    All Messages Received by {letters.name}
                </h1>
                {letters.message && letters.message.map((message, index) => (
                    <div key={index} >
                        <div className="mt-4 my-1 p-4 bg-white border rounded shadow-md">
                            {message.text}
                        </div>
                        <p className="ml-4">Date Sent: {message.date}</p>
                    </div>
